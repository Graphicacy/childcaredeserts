...
const createValidator = (expectedProperties) => {
  ...
  const validationKeys = _.keys(expectedProperties);
  ...
  return (scrutinizedObject) => {
    ...
    if (!scrutinizedObject) {
      return false;
    }

    // trying to keep things fast here... :/
    for (const currentKey in validationKeys) {
      ...
      const validationKey = validationKeys[currentKey];
      ...
      if (!_.hasIn(scrutinizedObject, validationKey)) {
        return false;
      }

      // get the optional validation function.
      const validator = expectedProperties[validationKey];
      if (!validator) {
      ...
      const propertyToValidate = scrutinizedObject[validationKey];
      const isValid = validator(propertyToValidate);
      if (isValid === false) {
        return false;
      }
    }
